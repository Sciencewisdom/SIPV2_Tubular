...
class HRFDataset(Dataset):
    def __init__(self, root_dir, split='train', img_size=512, augment=True, val_indices=None):
        super().__init__()
        self.root_dir = root_dir; self.split = split
        self.img_size = img_size; self.augment = augment
        img_dir = os.path.join(root_dir, 'training', 'images')
        gt_dir = os.path.join(root_dir, 'training', 'manual1')
        mask_dir = os.path.join(root_dir, 'training', 'mask')
        # Try multiple extensions
        all_image_paths = sorted(glob.glob(os.path.join(img_dir, '*.JPG')) + glob.glob(os.path.join(img_dir, '*.jpg')))
        all_gt_paths = sorted(glob.glob(os.path.join(gt_dir, '*.tif')) + glob.glob(os.path.join(gt_dir, '*.png')))
...
def get_hrf_loaders(root_dir, img_size=512, batch_size=2, num_workers=2, val_indices=None):
    train_dataset = HRFDataset(root_dir=root_dir, split='train', img_size=img_size, augment=True, val_indices=val_indices)
    val_dataset = HRFDataset(root_dir=root_dir, split='val', img_size=img_size, augment=False, val_indices=val_indices)
