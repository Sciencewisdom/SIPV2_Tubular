...
class STAREDataset(Dataset):
    def __init__(self, root_dir, split='train', img_size=512, augment=True, val_indices=None):
        ...
        all_image_paths = sorted(glob.glob(os.path.join(img_dir, '*.ppm')))
        all_gt_paths = sorted(glob.glob(os.path.join(gt_dir, '*.ah.ppm')))
        ...
        if val_indices is None:
            val_indices = list(range(10, 20))  # 20 images, last 10 as val
...
def get_stare_loaders(root_dir, img_size=512, batch_size=2, num_workers=2, val_indices=None):
    train_dataset = STAREDataset(root_dir=root_dir, split='train', img_size=img_size, augment=True, val_indices=val_indices)
    val_dataset = STAREDataset(root_dir=root_dir, split='val', img_size=img_size, augment=False, val_indices=val_indices)
