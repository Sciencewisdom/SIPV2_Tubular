...
def download_file(args):
    ...
    for attempt in range(max_retries):
        ...
        # Use timeout command to forcefully kill wget after 90s
        cmd = [
            'timeout', '--signal=KILL', '90',
            'wget', '-q', '--timeout=30', '-t', '1', '-O', out_path, url
        ]
        try:
...
def main():
    ...
    # Pre-clean zero-byte and tiny files
    cleaned = 0
    for url, out_path in all_tasks:
        if os.path.exists(out_path) and os.path.getsize(out_path) < 100000:
            ...
            cleaned += 1
    if cleaned:
        print(f"Cleaned {cleaned} partial/zero-byte files")
